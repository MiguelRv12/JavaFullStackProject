...
async function deleteAutorFromBD(id) {
    await fetchFromAPI(`/autores/${id}`, "DELETE");
}

async function saveAutor(autor) {
    const url = isNew() ? "/autores" : `/autores/${getLibroId()}`;
    const metodoType = isNew() ? "POST" : "PUT";

    await fetchFromAPI(url, metodoType, autor);
...
async function getLibroById(id) {
    return await fetchFromAPI(`/autores/${id}`);
}
